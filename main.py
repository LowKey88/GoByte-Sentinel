...
def fix_masternode(data_folder):
    try:
        ...
        if not os.path.isfile(wallet_file):
            print('It seems like the data folder (the one containing wallet.dat) is not the same as the folder where gobyte.conf is')
            data_folder = input('Please, write the data folder path: ')
            return fix_masternode(data_folder)

        print(colored('Make a copy of "wallet.dat" and "gobyte.conf" before continuing.\nThis program will try it best not to touch them, but just in case!', attrs=['bold']))
        ...
        for filename in os.listdir(data_folder):
            if filename in ('wallet.dat', 'gobyte.conf'):
                continue
        ...
        print(colored('Checking gobyte.conf contents', 'green'))
        tokens = gobyte_config.GoByteConfig.tokenize(config.gobyte_conf)
        ...
        try:
            ...
            r = None
            while r not in ('y', 'n'):
                print(colored('It seems like your gobyte.conf is not setting the wallet as masternode!', 'red'))
        ...
        if 'masternodeprivkey' not in tokens:
            print(colored('Your gobyte.conf does not contain a \'masternodeprivkey\' entry, please set it up before opening your wallet', 'red'))

        # Rewrite config
        with open(config.gobyte_conf, 'w') as fp:
            for k, v in tokens.items():
                fp.write('{}={}\n'.format(k, v))

        print(colored('Done checking\n', 'green'))
        print(colored('Please open your wallet (gobyted on linux) and wait for it completely load/open before continuing', attrs=['bold']))
        ...
        print(colored('If it says it can not connect, check your gobyte.conf settings', 'yellow'))
...
if __name__ == '__main__':
    ...
    print(colored('Using gobyte.conf: {}'.format(config.gobyte_conf), 'green'))
    ...
    if option == 1: 
        ...
        data_folder = os.path.dirname(config.gobyte_conf)
