...
# shims 'til we can fix the gobyted side
def SHIM_serialise_for_gobyted(sentinel_hex):
    ...
    # gobyted expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]
...
# shims 'til we can fix the gobyted side
def SHIM_deserialise_from_gobyted(gobyted_hex):
    from models import DESIRED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(gobyted_hex)
