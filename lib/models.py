...
class GovernanceObject(BaseModel):
    ...
    def import_gobject_from_gobyted(self, gobyted, rec):
        ...
        # shim/gobyted conversion
        object_hex = gobytelib.SHIM_deserialise_from_gobyted(object_hex)
        objects = gobytelib.deserialise(object_hex)
        subobj = None
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/GoByteDrive, etc.)
        return False
    ...
    def prepare(self, gobyted):
        try:
            object_hash = gobyted.rpc_command(*self.get_prepare_command())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, gobyted):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - gobyted.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, gobyted):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - gobyted.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, gobyted):
        now = int(time.time())
        return (self.created_at < (now - gobyted.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, gobyted):
        if self.is_expired(gobyted):
            return False

        return True

    def is_deletable(self, gobyted):
        if self.is_expired(gobyted):
            return True
